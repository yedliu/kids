...
class MakeDataForStageEvaluation {
  public data: {
    questions: any[];
  };
  ...
  public initial(data: any) {
    ...
    // console.log('this.initial(data): ', data, copy, copy.questionList);
    this.data.questions = this.initialQuestionList(copy.questionList);
  }
  ...
  public initialQuestion(data: {
    ...
    difficulty: number;
    examPointNameList: any[];
    id: number;
    knowledgeIdList: any[];
    knowledgeNameList: any[];
    optionElementList: any[];
    optionElementType: any[];
    optionList: any[];
    parentName: string;
    parentSerialNumber: string;
    problemElementList: any[];
    questionType: string;
    score: number;
    stuGetScore: number,
    serialNumber: number;
    ...
    let question: any = {
      ...
      type: this.normalizeQuestionType(data.questionType, data.layoutStyle),
      // analysis: this.normalizeQuestionAnalysis(output.analysis),
      correctResult: data.correctResult,
      analysisImgUrl: data.analysis,
      knowledgeNameList: data.examPointNameList,
      answerList: data.answerList
    };
    if (question.type === 'reorder' && data.optionElementList && data.optionElementList[0]) {
      if (data.optionElementList[0].optionElementType === 2) {
        question.type = 'reorderText';
      }
      if (data.optionElementList[0].optionElementType === 3) {
    ...
    if (type === 'singleSelect') {                                                                      // 单选题
      ...
      const classList: any = children.map((classType: any) => {
        ...
        optionItem && optionItem.forEach((element: any) => {
          option.push({
            id: element.id,
            type: element.type,
      ...
      question.analysis = output.analysis;
      // console.log('配对题格式化完成', data);
    } else if (type === 'textMatchImg' || type === 'textMatchText') {                                   // 配对题
      console.log('配对题格式化开始', data);
      const firstList: any[] = [];
...
export function normalizeQuestionTitle(t: any, type: string): {
  ...
  let nodeList: Node[] = string2node(t.title);
  console.log(nodeList);
  const Node: any = nodeList[0];
  if (Node) {
    switch (Node.nodeType) { // 解决没有p标签包裹，且第一个Node为text类型，无法insertBefore的问题
      case 3:
        nodeList = string2node(`<p>${t.title}</p>`);
        break;
      default:
        Node.insertBefore(string2node(`<span style="display: inline-block;vertical-align: top;font-weight: bold">【${t.type}】</span>`)[0], nodeList[0].childNodes[0]);
        break;
    }
  }
...
export default MakeDataForStageEvaluation;
