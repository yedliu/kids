<template>
  <board title="课堂内容" class="class-content">
    <div class="class-content__inner">
      <iframe v-if="data.length > 0" :src="url" frameborder="0" name="class-content-iframe" class="player__zml"></iframe>
    </div>
    <div class="class-content__player-decorate">
...
<script lang='ts'>
  ...
  export default class ContentClass extends Vue {
    ...
    private created() {
      this.$log('课件组件created', 'white', 'red');
    ...
    private async initIframe() {
      ...
      const jsonList: any[] = [];
      this.$log('init iframe', 'white', 'red');
      this.postMessage({
        action: 'setUserInfo',
        data: {
    ...
    private getMessageFromChild() {
      this.$log('init event listener', 'white', 'red');
      window.addEventListener('message', (e: any) => {
        this.$log('event', 'white', 'red');
        console.log(e);
        const data: any = e.data;
        this.$log('消息接收', 'white', 'red');
        this.$log(data.action, 'white', 'red');
        console.log(data);
        if (data) {
          const { action } = data;
          switch (action) {
            case 'ready':
              this.$log('course player ready', 'white', 'red');
              this.initIframe();
              break;
            case 'returnPageNumber':
              this.$log('returnPageNumber', 'white', 'red');
              // console.log(data);
    ...
    private handleBackClick() {
      this.$log(this.currentPage, 'white', 'red');
      this.$log(this.totalPage, 'white', 'red');
