  ...
  <div v-if="hasReportData" class="report-evaluation report__content">
    <div class="report-evaluation__content">
      ...
      <div v-show="active === 1">
        ...
        <!--课堂内容：zml 开始-->
        <course-content key="zml-coursecontent" v-if="courseContent.data.length > 0 && !courseContentVideo" class="report-evaluation__item" v-bind="courseContent"></course-content>
        <!--课堂内容 结束-->

        <!--zmg课程预告 开始-->
        <course-advance class="report-evaluation__item" v-if="courseAdvanceContentVideo" :url="courseAdvanceContentVideo" :desc="courseAdvanceContentDescription" :poster="courseAdvanceContentVideoPoster"></course-advance>
        <!--课程预告 结束-->

        <!--zml课程预告 开始-->
        <course-advance-zml class="report-evaluation__item" v-if="courseAdvanceContent.data.length > 0 && !hasAdvanceZmgCourseContent && !(courseContent.data.length > 0 && !hasZmgCourseContent)" v-bind="courseAdvanceContent" :desc="courseAdvanceContentDescription"></course-advance-zml>
        <!--课程预告 结束-->
...
<script lang='ts'>
  ...
  @Component({
    components: {
      ...
      Info: () => import('../evaluation/modules/info.vue'),
      Performance: () => import('../evaluation/modules/performance_smallClass.vue'),
      Interaction: () => import('../evaluation/modules/interaction_smallClass.vue'),
      InteractionZml: () => import('../evaluation/modules/interaction_smallClass_zml.vue'),
      Activity: () => import('../evaluation/modules/activity_1v1.vue'),
      Analysis: () => import('../evaluation/modules/analysis_smallClass.vue'),
      CourseContent: () => import('../evaluation/modules/content_class.vue'),
      CourseContentZmg: () => import('../evaluation/modules/content_smallClass.vue'),
      Introduction: () => import('../evaluation/modules/introduction_1v1.vue'),
      Methods: () => import('../evaluation/modules/methods_1v1.vue'),
      Comment: () => import('../evaluation/modules/comment_1v1.vue'),
      CourseAdvance: () => import('../evaluation/modules/course_advance.vue'),
      CourseAdvanceZml: () => import('../evaluation/modules/course_advance_zml.vue')
    }
  })
  ...
  export default class Evaluation extends Vue {
    ...
    private hasAdvanceZmgCourseContent = false;

    private courseContentVideo: string = '';
    private courseContentVideoPoster: string = '';
    private courseAdvanceContentVideo: string = '';
    private courseAdvanceContentVideoPoster: string = '';

    private courseAdvanceContentDescription: string = '';
    ...
    private advanceCourseContentZmg: string = '';
    ...
    private async getInfo() {
      this.loading = true;
      try {
        const info: any = await this.$api['report/formal-small-class']({param: this.$route.query.data}, {
          dialog: false
        });
        ...
        if (info.nextKidLessonCswareVO) {
          this.getAdvanceCourseVideo(info.nextKidLessonCswareVO);
        } else if (info.nextCourseWareContentDTO) {
          const courseWareContentList: any[] = [];
          this.courseAdvanceContentDescription = info.nextCourseWareContentDTO.description || '让我们来看看下节课的精彩内容吧～';
          const nextCourseWareContentDTO = info.nextCourseWareContentDTO;
          nextCourseWareContentDTO.cwType = 2;
          courseWareContentList.push(nextCourseWareContentDTO);
          this.getAdvanceCourseInfo(courseWareContentList);
        }
        if (info.parentAssessCaseDTO) {
    ...
    private async getCourseInfo(info: any[]) {
      ...
      console.log(info, this.courseContent);
    }
    ...
    private async getAdvanceCourseVideo(info: any) {
      if (!info) {
        return;
      }
      this.courseAdvanceContentVideo = info.videoUrl;
      this.courseAdvanceContentVideoPoster = info.videoPoster;
      this.courseAdvanceContentDescription = info.description || '让我们来看看下节课的精彩内容吧～';
    }

    private async getAdvanceCourseInfo(info: any[]) {
      if (!info) {
        return;
      }
      const zmlCourseList: any[] = info.filter((course: any) => {
        return course.cwType === 2;
      });
      if (zmlCourseList.length > 0) {
        this.courseAdvanceContent.lessonId = this.lessonId;
        this.courseAdvanceContent.data = zmlCourseList;
        this.courseAdvanceContent.user.studentName = this.baseInfo.studentName;
        this.courseAdvanceContent.user.studentIcon = this.baseInfo.studentAvatar;
        this.courseAdvanceContent.user.userId = this.baseInfo.studentName;
        this.courseAdvanceContent.user.lessonId = this.lessonId;
      }
      console.log(info, this.courseAdvanceContent);
    }
